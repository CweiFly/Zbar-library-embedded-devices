...
static inline void qr_handler(zbar_image_scanner_t *iscn)
{
    ...
    assert(line); // 如果line指针创建失败，则终止程序
...
{
    // mysize = sizeof(zbar_image_scanner_t);
    zbar_image_scanner_t *iscn = calloc(1, sizeof(zbar_image_scanner_t)); // 分配空间
    // zbar_image_scanner_t *iscn = malloc(sizeof(zbar_image_scanner_t)); // 分配空间
    if (!iscn)
        return (NULL);
...
//    iscn->time = (abstime.tv_sec * 1000) + ((abstime.tv_usec / 500) + 1) / 2;
    ...
    if (density > 0)
    {
        ...
        while (y < h)
        {
            ...
            {
                uint8_t d = *p;
                movedelta(1, 0); // x移动1位，y不动，指针指向这行的下一个像素
                zbar_scan_y(scn, d);
            }
            ASSERT_POS;
            quiet_border(iscn);
